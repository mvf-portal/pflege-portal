...
„Knowledge-Hub Pflege & Langzeitversorgung" — ein Rechercheportal zum Themenfeld Pflege & Langzeitversorgung. Ein Angebot von **Monitor Versorgungsforschung** (Betreiber: eRelation AG – Content in Health, Bonn).

Live: https://pflege.m-vf.de/
...
py vorlage-abgleich.py ..\pflege-portal --uebernehmen
```
...
| Live prüfen | `curl -s "https://pflege.m-vf.de/?cb=$(date +%s)"` — Cache-Buster nötig |
| Pages-Status | `gh api repos/mvf-portal/pflege-portal/pages/builds/latest` |
...
|---|---|---|
| `PFLEGEHUB` | Claude-API für die Studienauswahl | Der tägliche Lauf bricht ab, `index.html` bleibt unverändert |
| `PFLEGEHUBMC` | Mailchimp-API für den Kampagnen-Entwurf | Nur dieser Schritt entfällt (`continue-on-error`) |
...
**Alle Portale schreiben in dasselbe Mailchimp-Konto.** `mailchimp_entwurf.py` erkennt seine eigenen Kampagnen am Titel-Präfix `MVF Pflege-Newsletter`. Der Präfix muss sich **vollständig** von den Schwesterportalen unterscheiden: `datum_aus_titel()` prüft mit `startswith()`, ein bloßer Zusatz reicht nicht.
